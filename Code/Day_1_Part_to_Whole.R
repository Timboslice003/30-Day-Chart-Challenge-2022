...
myplot <- qbs_in_sb %>% ggplot(aes(x = 1, fill = as.factor(category))) +
  ...
  annotate(geom = 'text', x = 1.05, y = 4.25, label = 'Other (6/22)', 
           size = 5, color = 'white') +
  nflplotR::geom_nfl_headshots(aes(x = 1, y =8.5, player_gsis = '00-0019596'),
                               width = .23) +
  annotate(geom = 'text', x = 1.23, y = 9.3, label = 'Tom Brady (6/22)',
           size = 5, color = 'white') +
  annotate(geom = 'text', x = 1, y = 16.8, label = 'Rookie Deal QBs (10/22)',
           size = 5, color = 'white') +
  geom_image(aes(x = 1.1, y = 18.5, image = 'NFL_Draft_logo.png'), size = .2)

#Add Stadium Background and title
full_plot <-ggbackground(myplot ,background =  'SoFi.jpg',)

cowplot::ggdraw(full_plot) +
  theme(plot.title = element_text(color = 'white', face = 'bold', size = 24),
        plot.subtitle = element_text(color = 'white', face = 'bold', size = 14),
